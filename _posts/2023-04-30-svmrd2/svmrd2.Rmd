---
title: "Support Vector Machine with Radial Basis Kernel using Package kernlab"
description: |
...
- `kpar` : Kernel 함수에 포함된 초모수(Hyperparameter) 조합의 리스트(List)
    - `kernel = "rbfdot"`에 대해,
        - `sigma` : 차수
        - 만약 직접 입력하지 않으면 자동으로 최적의 값을 찾아준다.
- `prob.model` : `Test Dataset`에 대한 `예측 확률`의 생성 여부
    - `TRUE` : 함수 `predict()`를 이용하여 `Test Dataset`에 대한 `예측 확률`을 생성할 수 있다.


```{r}
# 옵션 kpar를 이용하여 sigma 값 입력
