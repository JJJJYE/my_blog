...
`Result!` `Number of Support Vectors`는 결정경계와 가까이 위치한 case의 수이다. 해당 데이터에서는 총 186개의 case로, `"Personal.Loan = no"`에 해당하는 case는 92개, `"Personal.Loan = yes"`에 해당하는 case는 94개이다. case의 행 번호는 `svm.model.li$index`를 이용하여 확인할 수 있다.
