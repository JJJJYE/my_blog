...
\eta_{it} &= \log{\rho_{it}} = b_{0} + u_{i} + v_{i} + \gamma_{t} + \phi_{t} + \sum_{j} \beta_{j}x_{jit}
\end{align*}
...
- $\log(치안시설수)$가 1 증가할 때, 상대적으로 위험은 $\exp(-0.145*1)=0.87$ 증가한다.
- $\log(안심귀가스카우트 이용수)$가 1 증가할 때, 상대적 위험은 $\exp(0.001*1)=1.00$ 증가한다.
- 여성인구비가 0.01 증가할 때, 상대적 위험은 $\exp(-16.244*0.01)=0.85$ 증가한다.
...
```{r, echo=FALSE}
beta <- model.nd$marginals.fixed$ln_sec_fac 
inla.pmarginal(0, beta)      # P(beta < b|y)
```

&#10148; 치안시설 수에 대한 회귀계수의 Posterior probability가 80%이상이므로 치안시설 수는 5대강력범죄에 유의하다.
즉, 치안시설수가 감소할수록 5대강력범죄의 위험은 증가한다.
...
```{r, echo=FALSE}
beta <- model.nd$marginals.fixed$ln_safe_return_use 
...
```{r, echo=FALSE}
beta <- model.nd$marginals.fixed$pop_female_rate 
inla.pmarginal(0, beta)      # P(beta < b|y)
```

&#10148; 여성인구비에 대한 회귀계수의 Posterior probability가 80%이상이므로 여성인구비는 5대강력범죄에 유의하다.
즉, 여성인구비가 감소할수록 5대강력범죄의 위험은 증가한다. 이것은 5대강력범죄의 피해자는 강간강제범죄를 제외하고 남성의 비율이 높은 것으로부터 유추할 수 있다.
