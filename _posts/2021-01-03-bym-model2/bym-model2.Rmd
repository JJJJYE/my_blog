...
description: |
  BYM Model including risk factor in Statial Modeling 
author:
  - name: Yeongeun Jeon
date: 01-03-2021
preview: preview.PNG
...
>INLA는 잠재 가우스 모형 (latent gaussian model)에 특화된 방법이며 복잡한 사후 분포를 단순화된 라플라스 근사치를 이용하고 적분을 유한개의 가중 합으로 계산하기 때문에 MCMC에 비해 비교적 짧은 계산시간에 정확한 결과를 제공한다는 이점이 있다. 이러한 장점을 이용하여 2019년 서울에서 발생한 강간강제추행을 분석하기 위해 [R-INLA (Integrated Nested Laplace Approximations)](https://www.r-inla.org/)를 적용해보았다. 

>이번에는 서울 강간강체주행범죄에 영향을 미칠 수 있는 요인으로 치안시설 수, 안심귀가스카우트 이용수, 여성인구비를 고려해보았다. 데이터는 [서울 열린데이터광장](https://data.seoul.go.kr/)에서 수집하였으며, BYM Model을 이용하여 공간 분석을 실시하였다. 
...
where $n_{i}$는 $i$ 번째 지역의 인구수. (참고 : [Bayesian Disease Mapping: Hierarchical Modeling in Spatial Epidemiology](https://www.routledge.com/Bayesian-Disease-Mapping-Hierarchical-Modeling-in-Spatial-Epidemiology/Lawson/p/book/9781138575424) p.85)
...
&#10148; 치안시설 수에 대한 회귀계수의 Posterior probability가 80%이상이므로 치안시설 수는 강간강제추행에 유의하다. 즉, 치안시설수가 많은수록 강간강제추행은 늘어난다.
...
단순히 공간효과만 비교하는 것보다 초과 확률(Exceedance probabilities)을 비교하는 것도 용이하다. 왜냐하면 초과 확률은 불확실성(Uncertainty)을 고려하기 때문이다.
