---
title: "Nearest Neighborhood Method using Package caret"
description: |
  Description for KNN algorithm using Package caret
author:
  - name: Yeongeun Jeon
  - name: Jung In Seo
date: 2023-03-10
...
set.seed(200)                                         # For CV
knn.fit <- train(Personal.Loan ~ ., data = UB.trd, 
...
`Result!` 랜덤하게 결정된 3개의 `k` 값에 대한 정확도를 보여주며, `k` = 5일 때 정확도가 가장 높은 것을 알 수 있다. 따라서 그리드 검색을 통해 찾은 최적의 초모수 값 5 근처의 값들을 탐색 범위로 설정하여 훈련을 다시 수행할 수 있다.

```{r}
customGrid <- expand.grid(k = seq(3, 10, by = 1))     # k의 탐색 범위 

set.seed(200)                                         # For CV
knn.tune.fit <- train(Personal.Loan ~ ., data = UB.trd, 
                      trControl = fitControl,
                      method = "knn", 
                      tuneGrid = customGrid,
                      preProc = c("center", "scale")) # Standardization for 예측 변수
...
`Result!` `k` = 3일 때 정확도가 가장 높다는 것을 알 수 있으며, `k` = 3을 가지는 모형을 최적의 훈련된 모형으로 선택한다.
...
plotLift(test.knn.prob, ac.numeric, cumulative = T, n.buckets = 24)   # plotLift(6-2에서 생성한 예측 확률, 실제 class)
