---
title: "Elastic Net Regression using Package glmnet"
description: |
  Description for Elastic Net Regression using Package glmnet
author:
  - name: Yeongeun Jeon
  - name: Jung In Seo
date: 2023-08-19
...
$$
\begin{align*}
\log{\frac{p}{1-p}} = &-2.469 + 0.624 Z_{\text{Income}} + 0.090 Z_{\text{CCAvg}} + 0.912 X_{\text{CD.Account1}}
\end{align*}
$$
...
$$
\begin{align*}
\log{\frac{p}{1-p}} = &-4.518 + 1.556 Z_{\text{Income}} -0.218 X_{\text{Family2}} + 0.805 X_{\text{Family3}} + 0.730 X_{\text{Family4}}   \\
                      &+ 0.254 Z_{\text{CCAvg}} + 1.506 X_{\text{Education2}} + 1.488 X_{\text{Education3}} + 0.019 Z_{\text{Mortgage}} \\
                      &-0.168 X_{\text{Securities.Account1}} + 2.240 X_{\text{CD.Account1}} -0.414 X_{\text{CreditCard1}}
\end{align*}
$$
...
```{r}
CM   <- caret::confusionMatrix(as.factor(test.elast.class), UB.ted$Personal.Loan, 
...
test.elast.prob %>%                                    # "Personal.Loan = yes"에 대한 예측 확률                           
  as_tibble
```
...
plotLift(test.elast.prob, UB.ted$Personal.Loan, cumulative = T, n.buckets = 24) # plotLift(7-2에서 생성한 예측 확률, 실제 class)
