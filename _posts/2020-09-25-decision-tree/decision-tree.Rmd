...
의사결정나무모형을 형성하기 위해 사용될 첫번째 Package는 `"rpart"`이다. `"rpart"`는 약간 수정된 CART를 사용하며, CP (Complexity Parameter)를 중심으로 분석한다. 게다가 `"rpart"`는 Cross Validation을 이용하여 의사결정나무를 생성하며, defalut값은 10-fold Cross Validation이다. 또한 가독성 좋은 그래프가 있기 때문에 트리를 시각화하기에 좋다.
